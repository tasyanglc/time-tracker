...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    private fun ensureProfileImageColumnExists(db: SQLiteDatabase) {
        try {
            db.execSQL("ALTER TABLE $TABLE_NAME ADD COLUMN $COLUMN_PROFILE_IMAGE TEXT")
        } catch (e: Exception) {
    ...
    fun getUserDetails(usernameOrEmail: String): Map<String, String>? {
        return try {
            val db = this.readableDatabase
            ensureProfileImageColumnExists(db)
            val cursor = db.rawQuery(
                "SELECT $COLUMN_USERNAME, $COLUMN_EMAIL, $COLUMN_PROFILE_IMAGE FROM $TABLE_NAME WHERE $COLUMN_USERNAME = ? OR $COLUMN_EMAIL = ?",
            ...
            if (cursor.moveToFirst()) {
                userDetails = mapOf(
                    ...
                    "profile_image" to (cursor.getString(2) ?: "")
                )
            }
            cursor.close()
            userDetails
    ...
    fun updateUser(oldUsernameOrEmail: String, newUsername: String, newEmail: String, newPassword: String, profileImage: String?): Boolean {
        return try {
            val db = this.writableDatabase
            ensureProfileImageColumnExists(db)
            val values = ContentValues()
            ...
            val rows = db.update(TABLE_NAME, values, "$COLUMN_USERNAME = ? OR $COLUMN_EMAIL = ?", arrayOf(oldUsernameOrEmail, oldUsernameOrEmail))
            rows > 0
        } catch (e: Exception) {
            e.printStackTrace()
            false
    ...
    fun deleteUser(usernameOrEmail: String): Boolean {
        return try {
            val db = this.writableDatabase
            val rows = db.delete(TABLE_NAME, "$COLUMN_USERNAME = ? OR $COLUMN_EMAIL = ?", arrayOf(usernameOrEmail, usernameOrEmail))
            rows > 0
        } catch (e: Exception) {
            e.printStackTrace()
            false
